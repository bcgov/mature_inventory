...
date: "Version 2026-05-14; Created `r Sys.Date()`"
#runtime: shiny
...
```
<br>
<div style="font-weight: bold; text-align: left;"> Figure 2. Mature Sample Distribution by BEC subzone/variant. </div>
...
#### 1. The number of mature ground samples and their most recent measurement years are summarized.
...
#### 2. Leading species and species composition from mature ground samples (“ground”) are compared with those from the Vegetation Resources Inventory (VRI) forest inventory coverage (“inventory”).
...
#### 3. 95% confidence intervals for the Ratio Of Means (ROM) and a 10% Region of Practical Equivalence (ROPE) were used to assess the level of agreement in forest attributes between ground samples and the VRI.
...
| **Overall Ratio of means (ground/inventory), 95% confidence interval, & ROPE* interval for the live attributes age, height, basal area & volume, across each sample design.**
...
#### 4. Forest attributes showing statistically significant differences or agreement are identified.
...
```{r, fig.height=2.5,echo=FALSE,warning=FALSE, message = FALSE}
stockplot_stem() + theme_bw(12) +
    theme(
        #axis.line = element_line(colour="darkgray"), 
        panel.grid.major.y = element_line(color = 'darkgray'), 
        panel.grid.major.x = element_blank(),
...
| **Figure 5. Live whole stem volume (m3/ha) and total stems/ha by DBH class, including all standing trees >= 4cm DBH at the latest measurement.**
  
  
<br>

```{r, fig.height=4,fig.width=5,echo=FALSE,warning=FALSE, message = FALSE}
smalltrplot() + theme_bw(12) +
    theme(
        #axis.line = element_line(colour="darkgray"), 
        panel.grid.major.y = element_line(color = 'darkgray'), 
        panel.grid.major.x = element_blank(),
...
| **Figure 6 under the small tree report “Figure 6. Live total stems/ha by height class, including all standing trees <4cm DBH at the latest measurement.**
  
<br> 

### BA, Age, Height, and Volume

`r htmltopdf(scatter_text())`
...
| **Figure 8. Components of Bias (total, model, attribute), for Basal Area (left graph) and Net Merchantable Volume (right graph), by ground sample design.**


## General Notes

### Tables of Mature GRID samples
...
### General Notes / Assumptions / References

`r htmltopdf(gn())`

### Tree Species and Damage Agents
...
DT::datatable(reference, escape = FALSE, rownames = FALSE)
```
